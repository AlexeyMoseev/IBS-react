...
const Header: FC<IHeader> = (props) => {
	const cls = [s.searchField, s[props.type]];
	const { data__getFiltredItems } = useActions();
	return (
		<header className={s.header}>
			<form className={cls.join(' ')}>
				<button disabled className={s.searchButton}>
					<svg
				...
				<input
					onChange={data__getFiltredItems}
					className={s.searchInput}
					placeholder='Search products'
				/>
			</form>
			<div className={s.accountButtons}>
				<button className={s.shoppingCart}>
					<svg
						width='24'
						height='24'
						viewBox='0 0 24 24'
				...
				<button className={s.accountCircle}>
					<svg
						width='24'
						height='24'
						viewBox='0 0 24 24'
